...
const handleAsset = function(arr) {
    return arr.map(function(e) {
        ...
        obj.js = e.js.map(function(e) {return e.match(/\/(\w+\.?\w+\.js$)/)[1]})
        obj.css = e.css
...
module.exports = function() {
    ...
    manifest.register ? ((registerObj.omnibox.keyword = manifest.register) && (config.omnibox = registerObj)) : null
    // config.default_locale = config.language || defaultConfig.default_locale
    manifest.devpage && manifest.devpage.endsWith('.html') ? (config.devtools_page=manifest.devpage) : null
    manifest.optionpage && manifest.optionpage.endsWith('.html') ? ((optionPage.page=manifest.optionpage) && (config.options_ui=optionPage)) : null
    config.homepage_url = manifest.home || defaultConfig.homepage_url

    manifest.scriptsAndCss && manifest.scriptsAndCss.length ? (config.content_scripts = handleAsset(manifest.scriptsAndCss)) : null

    /**handle background */
    if(manifest.background && manifest.background.page && manifest.background.page.endsWith('.html')) {
    ...
    if(manifest.background && manifest.background.scripts && manifest.background.scripts.length) {
        backgroundobj.scripts = manifest.background.scripts.map(function(e) { return e.match(/\/?(\w+\.?\w+\.js$)/)[1]})
    }
